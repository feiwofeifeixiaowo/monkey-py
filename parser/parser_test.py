...
class TestOperatorPrecedenceParsing(unittest.TestCase):
    def test_operator_precedence(self):
        ...
        tests = [OperatorTest("-a * b;", "((-a) * b)"),
                 ...
                 OperatorTest("a * [1, 2, 3, 4][b * c] * d;", "((a * ([1, 2, 3, 4][(b * c)])) * d)"),
                 OperatorTest("add(a * b[2], b[1], 2 * [1, 2][1]);", "add((a * (b[2])), (b[1]), (2 * ([1, 2][1])))"),
                 ]
...
class TestIfExpression(unittest.TestCase):
    def test_ifexpression(self):
        ...
        if not self.check_infixexpression(exp.condition, 'x', '<', 'y'):
            return None
...
class TestStringLiteralExpression(unittest.TestCase):
    def test_stringliteral(self):
        ...
        self.check_parsererrors(p)
        stmt = program.statements[0]
        if not isinstance(stmt.expression, StringLiteral):
            self.fail('exp not *ast.StringLiteral. got={}'.format(type(stmt.expression)))
        if stmt.expression.value != 'hello world':
            self.fail('literal.Value not {}. got={}'.format('hello world', stmt.expression.value))
...
class TestParsingArrayLiterals(unittest.TestCase):
    def test_array_parser(self):
        ...
        if not isinstance(array, ast.ArrayLiteral):
            self.fail('exp not ast.ArrayLiteral got={}'.format(type(array)))
        if len(array.elements) != 3:
            self.fail('len(array.Elements) not 3. got={}'.format(len(array.elements)))
        self.check_integer_literal(array.elements[0], 1)
        self.check_infixexpression(array.elements[1], 2, '*', 2)
        self.check_infixexpression(array.elements[2], 3, '+', 3)
...
class TestParsingIndexExpression(unittest.TestCase):
    def test_parse_index(self):
        ...
        if not isinstance(index_exp, ast.IndexExpression):
            self.fail('exp not ast.IndexExpression got={}'.format(type(index_exp)))
        if not self.check_identifier(index_exp.left, 'myArray'):
            return None
        if not self.check_infixexpression(index_exp.index, 1, '+', 1):
            return None
...
class TestParsingHashLiteralsStringKeys(unittest.TestCase):
    def test_parsing_hashliteral(self):
        ...
        if not isinstance(hash, ast.HashLiteral):
            self.fail('exp not ast.HashLiteral got={}'.format(type(hash)))
        expected = {'one':1,
                    'two':2,
                    'three':3}
        if len(hash.pairs) != len(expected):
            self.fail('hash.Pairs has wrong length. got={}'.format(len(hash.pairs)))
        for k,v in hash.pairs.items():
            if not isinstance(k, ast.StringLiteral):
                print('key is not ast.StringLiteral. got=%{}'.format(type(k)))
                continue
        expected_val = expected[k.string()]
        self.check_integer_literal(v, expected_val)
    ...
    def test_parsing_hashwithexp(self):
        ...
        tests = {
            'one': lambda e: self.check_infixexpression(e, 0, '+', 1),
            'two': lambda e: self.check_infixexpression(e, 10, '-', 8),
            'three': lambda e: self.check_infixexpression(e, 15, '/', 5),
        }
        for k,v in hash.pairs.items():
            literal = k
            if not isinstance(literal, ast.StringLiteral):
                print('key is not ast.StringLiteral. got={}'.format(type(literal)))
                continue
            t_func = tests.get(literal.string(), 'error')
            if t_func == 'error':
                print('No test function for key {} found'.format(literal.string()))
